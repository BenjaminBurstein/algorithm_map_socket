<template>
  <div class="text-center w-full flex mt-5">
    ...
    <p class="bg-white w-fit m-auto" v-else-if="selfUser.pos && selfUser.pos.lat && selfUser.pos.lng">
      ...
      <input
        @change="changeDestination()"
        v-if="destination.pos && destination.pos.lat && destination.pos.lng"
        type="time"
        id="picker"
        name="picker"
        v-model="destination.time"
...
export default {
  ...
  computed: {
    selfDistanceKm() {
      let distanceKm = null;
      if (
        this.selfUser.marker === null ||
        (this.selfUser.marker && this.selfUser.marker.pos === null) ||
        (this.selfUser.marker &&
          this.selfUser.marker.pos &&
          (this.selfUser.marker.pos.lat === null ||
            this.selfUser.marker.pos.lng === null))
      ) {
    ...
    selfDepartTime() {
      ...
      try {
        ...
        departureDateTime.setTime(
          destinationDatetime.getTime() - travelTimeMinute * 60000
        );
        //return Heure h minute avec le 0 qui se met s'il n'y a que un digit pour les minutes
        const today = new Date();
        const returnDate = !(
          departureDateTime.getMonth() === today.getMonth() &&
          departureDateTime.getDate() === today.getDate()
        )
          ? `le ${departureDateTime.getDate()} ${
              monthName[departureDateTime.getMonth()]
            } `
          : "";
        ...
        if (
          this.destination.pos === null ||
          (this.destination.pos &&
            (this.destination.pos.lat === null ||
              this.destination.pos.lng === null))
        ) {
  ...
  methods: {
    changeDestination() {
      this.$socket.changeDestination(this.destination);
    },
  },
};
</script>
